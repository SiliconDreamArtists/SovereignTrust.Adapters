...
First set `SDA_AZURESQL_LIVE_SERVER` and `SDA_AZURESQL_LIVE_DATABASE` to the **approved** integration target. Run `pwsh -NoProfile -File Src/Data/AzureSql/Tests/Launch-AzureSqlLiveIntegration.ps1 -TargetProbe` before setting live readiness flags. This read-only probe resolves the real protected resource through production hydration and lazy loading, authenticates with SqlClient, and verifies both connection-string target fields and `DB_NAME()`. Those environment variables only validate the target; they never redirect it. If the secret points elsewhere, keep the live suite blocked until an approved configuration solution exists. Do not change a shared secret to make the test pass.

Set `SDA_AZURESQL_LIVE_SCHEMA` to a dedicated schema beginning `sda_azure_sql_it_`, then run `pwsh -NoProfile -File Src/Data/AzureSql/Tests/Launch-AzureSqlLiveIntegration.ps1 -FixtureProbe`. This read-only preflight checks the fixture and visible permissions without readiness flags or mutation. Enable mutation only for an isolated integration database after both probes pass and after independently verifying Key Vault access, supported authentication, network/firewall access, and the test identity's database grants. Set `SDA_AZURESQL_LIVE_READY=1`, `SDA_AZURESQL_LIVE_DEDICATED=1`, `SDA_AZURESQL_LIVE_NETWORK_READY=1`, `SDA_AZURESQL_LIVE_IDENTITY_READY=1`, and `SDA_AZURESQL_LIVE_FIXTURES_READY=1` only after those checks. The identity needs SELECT and DELETE on the fixture table, EXECUTE and VIEW DEFINITION on the ingestion procedure, and no DDL or unrelated table write rights. The suite never creates or drops SQL objects.
...
The hydrated adapter jacket `Resource` must be a protected **connection string** with `Server`/`Data Source`, `Database`/`Initial Catalog`, and one of `Authentication=Active Directory Default` or `Authentication=Active Directory Managed Identity`. Managed Identity may include `User ID=<user-assigned identity client ID>`; omit it for a system-assigned identity. Default authentication does not accept `User ID`. SQL password, service principal, integrated security, and connection strings without an explicit supported authentication mode are not supported in this phase. Keep the resource in the protected secret, never in plans or application configuration. The jacket's `Addresses` is a Key Vault address and is never used as the SQL endpoint.
